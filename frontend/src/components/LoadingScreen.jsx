    ...
    return (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 select-none">
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-sky-900/15 rounded-full blur-[100px]" />
                <div className="absolute bottom-1/4 left-1/2 -translate-x-1/2 w-[400px] h-[400px] bg-indigo-900/10 rounded-full blur-[80px]" />
            </div>
            ...
            <div className="relative flex flex-col items-center gap-8 px-8 max-w-sm w-full">
                ...
                <div className="relative">
                    ...
                    <div className="absolute -inset-3 rounded-full border border-sky-500/15 animate-ping" style={{ animationDuration: '2.5s', animationDelay: '0.3s' }} />
                    <div className="relative w-24 h-24 rounded-full gradient-brand flex items-center justify-center shadow-glow">
                ...
                <div className="text-center">
                    ...
                    <p className="text-slate-500 text-sm mt-1 font-medium tracking-wide">Mensajeria segura</p>
                </div>
                            ...
                            <div key={step.key}
                                ...
                                <div className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center transition-all duration-500 ${
                                    isDone
                                        ? 'bg-sky-500'
                                        : isActive
                                            ? 'bg-slate-600 ring-2 ring-sky-500/50'
                                            : 'bg-slate-700'
                                }`}>
                                    {isDone ? (
                                ...
                                <span className={`text-sm font-medium ${
                                    isDone ? 'text-sky-300' : isActive ? 'text-slate-300' : 'text-slate-600'
                                }`}>
                                    {step.label.replace('...', '')}
                                </span>
                                {isDone && (
                                    <span className="ml-auto text-xs text-sky-400/70 font-semibold">✓</span>
                                )}
                                {isActive && (
                ...
                <div className="w-full">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-xs text-sky-400 font-bold tabular-nums">{pct}%</span>
                    </div>
            ...
            <div className="absolute bottom-6 text-center text-slate-700 text-xs font-medium tracking-wide">
                todos &copy; {new Date().getFullYear()}
            </div>
        </div>
    );
}
