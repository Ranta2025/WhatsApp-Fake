...
export default function UnblockAccount() {
    ...
    const handleStep1 = async (e) => {
        ...
        if (!gmail.trim()) {
            setError('Ingresa tu correo para continuar con la recuperación del acceso.');
            return;
        }
        ...
        try {
            ...
            const msg = (typeof data === 'string')
                ? data
                : data?.message || data?.error || 'No pudimos enviar el código a ese correo. Revisa la información e inténtalo nuevamente.';
            setError(msg);
        } finally {
            setLoading(false);
        }
    ...
    const handleStep3 = async (e) => {
        ...
        if (!newPassword.trim() || !confirmPassword.trim()) {
            setError('Completa ambos campos para definir tu nueva contraseña.');
            return;
        }
        ...
        try {
            ...
            alert('Tu cuenta ya está disponible nuevamente. Inicia sesión con tu nueva contraseña.');
            navigate('/login');
        } catch (err) {
            const data = err?.response?.data;
            const msg = (typeof data === 'string')
                ? data
                : data?.message || data?.error || 'No pudimos recuperar tu acceso en este momento.';
            setError(msg);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <AuthLayout
            ...
            subtitle={
                step === 1
                    ? 'Protegimos tu acceso temporalmente. Te ayudamos a recuperarlo en unos minutos.'
                    : step === 2
                    ? 'Confirma el código que enviamos a tu correo'
                    : 'Define una nueva contraseña para volver a entrar con seguridad'
            }
            footer={(
                <span>
        ...
        >
                ...
                <form onSubmit={handleStep1} className="space-y-4">
                    <div className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs text-slate-400 sm:grid-cols-3">
                        <div className="rounded-xl bg-slate-950/60 px-3 py-2.5">
                            <div className="uppercase tracking-[0.2em] text-rose-300/80">Cuenta</div>
                            <div className="mt-1 text-sm font-semibold text-slate-100">Acceso protegido</div>
                        </div>
                        <div className="rounded-xl bg-slate-950/60 px-3 py-2.5">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-2xl border border-white/10 bg-slate-950/70 p-3.5 pl-10 text-white placeholder-slate-500 transition-all duration-200 focus:border-rose-400 focus:outline-none focus:ring-1 focus:ring-rose-400"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="mt-6 w-full rounded-2xl bg-[linear-gradient(135deg,#ef4444,#be123c)] py-3.5 font-bold text-white shadow-[0_20px_50px_-20px_rgba(239,68,68,0.8)] transition-all duration-200 hover:scale-[1.01] hover:brightness-110 active:scale-[0.98] disabled:opacity-50"
                    >
            ...
            {step === 2 && (
                <form onSubmit={handleStep2} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-1.5">Código de Desbloqueo</label>
                        <div className="relative">
                            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                <svg className="w-5 h-5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <input
                                ...
                                className="w-full rounded-2xl border border-white/10 bg-slate-950/70 p-3.5 pl-10 text-center text-lg tracking-widest text-white placeholder-slate-500 transition-all duration-200 focus:border-rose-400 focus:outline-none focus:ring-1 focus:ring-rose-400"
                                placeholder="000000"
                                maxLength="20"
                            />
                        </div>
                    ...
                    <button
                        type="submit"
                        className="mt-6 w-full rounded-2xl bg-[linear-gradient(135deg,#ef4444,#be123c)] py-3.5 font-bold text-white shadow-[0_20px_50px_-20px_rgba(239,68,68,0.8)] transition-all duration-200 hover:scale-[1.01] hover:brightness-110 active:scale-[0.98]"
                    >
                        Verificar código
                    </button>
                    <button
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-2xl border border-white/10 bg-slate-950/70 p-3.5 pl-10 pr-12 tracking-wide text-white placeholder-slate-500 transition-all duration-200 focus:border-rose-400 focus:outline-none focus:ring-1 focus:ring-rose-400"
