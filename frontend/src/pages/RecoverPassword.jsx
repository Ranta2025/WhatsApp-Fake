...
export default function RecoverPassword() {
    ...
    const handleStep1 = async (e) => {
        ...
        setLoading(true);
        try {
            await api.post('/forgot-password-send', { email });
            setStep(2);
        } catch (err) {
    ...
    const handleStep3 = async (e) => {
        ...
        setLoading(true);
        try {
            await api.post('/forgot-password-change', { email, code, password: newPassword });
            ...
            const msg = (typeof data === 'string')
                ? data
                : data?.message || data?.error || 'No pudimos actualizar tu contrasena.';
            setError(msg);
        } finally {
            setLoading(false);
        }
    ...
    const titles = ['Recuperar contrasena', 'Verifica el codigo', 'Nueva contrasena'];
    const subtitles = [
        'Te ayudamos a recuperar tu acceso en pocos pasos',
        'Confirma el codigo que enviamos a tu correo',
        'Crea una contrasena nueva y vuelve a entrar'
    ];
    ...
    return (
        <AuthLayout
            ...
            footer={(
                <span>
                    Recuerdas tu contrasena? <Link to="/login" className="text-sky-400 hover:text-sky-300 font-medium">Inicia sesion</Link>
                </span>
            )}
        >
            {error && (
            ...
            {step === 1 && (
                <form onSubmit={handleStep1} className="space-y-5">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-white/[0.06] bg-slate-900/50 p-3 pl-10 text-white placeholder-slate-500 transition-all focus:border-sky-500/30 focus:outline-none focus:ring-1 focus:ring-sky-500/15"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 py-3 font-semibold text-white shadow-lg shadow-sky-500/10 transition-all duration-200 hover:shadow-sky-500/20 hover:brightness-105 active:scale-[0.98] disabled:opacity-50"
                    >
            ...
            {step === 2 && (
                <form onSubmit={handleStep2} className="space-y-5">
                    <div>
                        <label className="block text-sm font-medium text-slate-400 mb-2">Codigo de Recuperacion</label>
                        <div className="relative">
                            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                <svg className="w-5 h-5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-white/[0.06] bg-slate-900/50 p-3 pl-10 text-center text-lg tracking-widest text-white placeholder-slate-500 transition-all focus:border-sky-500/30 focus:outline-none focus:ring-1 focus:ring-sky-500/15"
                                placeholder="000000"
                                maxLength="20"
                            />
                        </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-white/[0.06] bg-slate-900/50 p-3 pl-10 pr-12 tracking-wide text-white placeholder-slate-500 transition-all focus:border-amber-500/30 focus:outline-none focus:ring-1 focus:ring-amber-500/15"
